...
//        String url = PRODUCT_PAGE;
        ...
        String url = siteMaps.getProperty(
                MyApplicationConstants.ProductFeature.VIEW_PRODUCT_PAGE);
        try {
            ProductDAO dao = new ProductDAO();
            dao.getAllProductList();
            List<ProductDTO> list = dao.getProductList();
            request.setAttribute("list", list); // Đặt danh sách sản phẩm vào thuộc tính "list" của request
        } catch (SQLException ex) {
            log("GetProductListServlet_SQL: " + ex.getMessage());
        } catch (NamingException ex) {
            log("GetProductListServlet_Naming: " + ex.getMessage());
